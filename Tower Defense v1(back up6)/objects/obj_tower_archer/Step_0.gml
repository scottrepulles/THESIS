...
if (_farthest != noone) && (ready_to_fire)
{
	...
	// Get the enemy's sprite height to aim at center of body
	var _enemy_height = sprite_get_height(_farthest.sprite_index);
	var _target_body_y = _farthest.y - (_enemy_height * 0.5); // Aim at center/body
	
	// Alternative: Use fixed offset for consistency
	// var _target_body_y = _farthest.y - 64; // Adjust 64 to your sprite size
	...
	{
		// ⭐ FIXED: Aim at the body position instead of feet
		image_angle = point_direction(x, y, _farthest.x, _target_body_y);
		...
		// ⭐ OPTIONAL: Store target for homing/tracking
		target = _farthest;
		target_y_offset = _enemy_height * 0.5; // Save offset for tracking
	}
